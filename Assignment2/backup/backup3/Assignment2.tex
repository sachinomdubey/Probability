...
\textbf{Finding the minimum distance between two skew lines.} \\ \\
\textbf{Step 1:} Perform the Steps 1 to 3 given for finding the closest points between two skew lines. \\ \\
\textbf{Step 2:} Using the distance formula, obtain the distance between points $\Vec{c_1}$ and  $\Vec{c_2}$. The obtained distance is the minimum distance between the two skew lines. \\ \\
\textbf{Solution :} \\
\textbf{Problem 1:} \\
The direction vectors of the lines are
$\vec{m_1}$=\myvec{3 \\ 2 \\6} and $\vec{m_2}$=\myvec{1 \\ 2 \\2}. \\
...
\textbf{Problem 2:} \\
The direction vectors of the lines are
$\vec{m_1}$=\myvec{1 \\ -1 \\-2} and $\vec{m_2}$=\myvec{3 \\ -5 \\-4}. \\
Thus, the angle $\theta$ between two vectors is given by 
%
\begin{align}
\label{eq:line_scalar_prod}
